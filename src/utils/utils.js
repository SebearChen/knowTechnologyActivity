...
//设置分享信息
export function setShareApp(shareString) {
    if(!window.PCJSKit){return;}
      //默认在APP里  window.PCJSKit
      if(isiOS()){
              window.PCJSKit.changeMeta(shareString)
      }else{
              window.PCJSKit.getSource("appshareMess/*@_HTML_META_START_"+shareString+"_HTML_META_END_@*/")
              setTimeout(function(){
                      window.PCJSKit.changeMeta(shareString)
              },1000)
              console.log("appshareMess/*@_HTML_META_START_"+shareString+"_HTML_META_END_@*/")
      }
  }
